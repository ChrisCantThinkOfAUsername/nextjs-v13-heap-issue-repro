# @module-federation/nextjs-mf heap overflow repro

## Steps
- Started with: https://github.com/module-federation/module-federation-examples/tree/master/nextjs-v13
- imported large packages to pages/dummyPage.js in /checkout
- duplicated pages/dummyPage.js 1000 times in /checkout using createPages.js

## Observation
- running `npm run build` in /checkout fails with "FATAL ERROR: Ineffective mark-compacts near heap limit Allocation failed - JavaScript heap out of memory"
- running `npm run dev` in /checkout succeeds 
- removing all references to NextFederationPlugin in /checkout then running `npm run build` succeeds
